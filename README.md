...
Every normal workspace on a monitor always shows all its app icons inside a rounded translucent card, including inactive workspaces. The active card has a tinted header, prominent workspace label and status dot; window counts remain in the header tooltip. Click a header to switch workspaces. Empty workspaces retain their header. Closed pinned launchers and **Other windows** (unknown/special membership) stay outside normal cards. Known windows on another monitor are excluded. Window actions and previews use only the item's members; hide and launcher pinning remain application-wide. Grouped dragging and redundant per-icon workspace labels are disabled.
...
Both dock layouts use rounded theme-colored icon tiles, accent window-count badges,
