...
TestCase {
  ...
  function makeController(edge) {
    return createTemporaryObject(controllerFactory, this, {
  ...
  function init() {
    writer.writes=[]
    writer.mode="accepted"
  }

  function test_pointer_moves_are_preview_only_and_release_commits_once() {
    var c=makeController("left")
    verify(c.beginResize(-1600))
    compare(c.resizeStartWidth,320)
    verify(c.updateResize(-1560))
    compare(c.geometry.width,360)
    compare(writer.writes.length,0)
    verify(c.updateResize(-1540))
    compare(c.geometry.width,380)
    compare(writer.writes.length,0)
    ...
    compare(writer.writes[0].value,380)
  ...
  function test_right_edge_uses_stable_global_delta() {
    ...
    verify(c.beginResize(-100))
    verify(c.updateResize(-140))
    compare(c.geometry.width,360)
    verify(c.updateResize(-160))
    compare(c.geometry.width,380)
    compare(writer.writes.length,0)
    c.finishResize(false)
    compare(writer.writes.length,1)
    compare(writer.writes[0].value,380)
  }
  ...
  function test_noop_cancel_and_grab_loss_write_nothing() {
    ...
    verify(handle !== null)
    handle.cancelActiveResize("grab-loss")
    compare(writer.writes.length,0)
    verify(!c.resizeActive)
  ...
  function test_conflicts_and_surface_changes_cancel_preview() {
    ...
    compare(writer.writes.length,0,"accepted collapse conflict cancels preview")

    c.settings=Object.assign({},c.settings,{sidebarCollapsed:false,sidebarExpandedWidth:320})
    tryCompare(c,"collapsed",false)
    verify(c.beginResize(0))
    c.updateResize(80)
    c.settings=Object.assign({},c.settings,{sidebarExpandedWidth:340})
    tryCompare(c,"resizeActive",false)
    compare(writer.writes.length,0,"accepted width conflict cancels preview")
  ...
  function test_rail_and_overlapping_interaction_disable_handle() {
    var c=makeController("left")
    c.settings=Object.assign({},c.settings,{sidebarCollapsed:true})
    tryCompare(c,"collapsed",true)
    verify(!c.beginResize(0))
    c.settings=Object.assign({},c.settings,{sidebarCollapsed:false})
