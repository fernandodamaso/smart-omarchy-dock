...
// Ctrl moves only the window into the clicked monitor's active workspace.
f.clear();assert.equal(c.activateTarget(target,true,'DP-1'),true)
assert.deepEqual(f.requests,['movetoworkspacesilent 1,address:0x1','focuswindow address:0x1'])
assert.equal(f.batches.length,0,'Ctrl on a window never moves its whole workspace')
f.clear();c.selectedConnector='HDMI-A-1';c.activateTarget(target,true,'HDMI-A-1')
assert.deepEqual(f.requests,['focuswindow address:0x1'],'already on the active workspace: focus only')
...
f.clear();c.activateTarget(c.captureTarget('ws'),false,'DP-1')
assert.deepEqual(f.requests,['workspace name:Design work']);assert.equal(f.batches.length,0,'plain header focuses in place')
f.clear();c.activateTarget(c.captureTarget('ws'),true,'DP-1')
assert.match(JSON.stringify(f.batches),/movecurrentworkspacetomonitor 0/,'Ctrl header pulls the workspace')
...
f.clear();a.minimizedOrigins={'0x1':{workspace:'name:Design work',monitor:'1'}}
c.activateTarget(target,true,'DP-1')
assert.match(JSON.stringify(f.batches),/movetoworkspace 1,address:0x1/,'Ctrl restores a minimized window into the active workspace')
