...
test_dir="$(mktemp -d -t smartdock-context-menu.XXXXXX)"
trap 'rm -rf -- "$test_dir"' EXIT
cp -R "$repo_root/components" "$test_dir/components"
cp "$repo_root/tests/runtime/context-menu-load.qml" "$test_dir/shell.qml"
...
grep 'context-menu-load: PASS' "$test_dir/output"
