...
Workspace-header activation still uses the central workspace-on-monitor route: clicking a grouped header pulls that workspace onto the clicked dock monitor and focuses it. A plain click on a window icon inside a workspace card focuses the exact window in place; Ctrl+left click explicitly pulls the card workspace to the clicked dock monitor before focusing it. Workspace monitor pins keep pinned workspaces on their owner monitor for header and Ctrl activation, while plain app-icon activation already focuses in place.
The pin-aware activation and header-menu work from FDM-942 and FDM-943 are integrated here; FDM-949 remains the full-host qualification reference.
