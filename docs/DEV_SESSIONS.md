...
| KVM guest (virtio-vga + guest Hyprland DRM) | **Display/capture feasibility PASS; pointer/focus contract UNQUALIFIED** — guest `grim` returns changing PNGs while host QEMU sits on inactive workspace `4`. Task 2/4 host pointer/focus-unchanged was not met; capture did not dispatch host pointer. |
