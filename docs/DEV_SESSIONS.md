...
| KVM guest (virtio-vga + guest Hyprland DRM) | **Display/capture PASS; pointer/focus PASS_WITH_ATTRIBUTION** — task8c (QEMU PID `1922406`, address `0x5559cc5246d0`, inactive ws `4`) completed three guest `wtype` inputs and three distinct `grim` frames with host focus/workspace unchanged. Cursor changes were correlated to passive X/Y events from physical Razer `event9`; the agent never moved or focused the host pointer/window. Literal coordinate equality remains a stricter, user-activity-sensitive check. Evidence: `~/.local/state/smartdock/dev-sessions/task8c/evidence/pointer-focus-gate-task8c-final.json` and `host-evdev-pointer-rerun.jsonl`. |
