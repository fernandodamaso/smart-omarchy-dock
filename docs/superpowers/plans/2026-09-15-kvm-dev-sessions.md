...
| 2. Lifecycle/placement | UNQUALIFIED (pointer/focus-unchanged NOT MET) | unit tests + live task2d (QEMU 1419058 / 0x5559cb52a2f0, SSH+packages, idempotent stop). host-before → host-after-setup: cursor 1029,1064 → 1122,558; active window chatgpt/ws1 → ghostty/ws2. Capture did not dispatch host pointer. Placement/SSH still evidenced. ~/.local/state/smartdock/dev-sessions/task2d/evidence/ |
...
| 4. Guest compositor/input/capture | UNQUALIFIED (pointer/focus-unchanged NOT MET) | unit tests + live task4a Virtual-1 grim (input 001≠002, labeled colors 006–008 distinct), QEMU 1470347 on ws 4. Input/color frames cursor 728,252→1307,513 and 1259,583→580,866; host window/ws stayed 1. Capture did not dispatch host pointer. Guest frames remain valid evidence. ~/.local/state/smartdock/dev-sessions/task4a/evidence/ |
