...
| 4. Guest compositor/input/capture | PASS_WITH_ATTRIBUTION | task8c guest `wtype` returned 0 for three operations and `grim` produced three distinct frames (`frame-004.png`..`frame-006.png`) while QEMU was inactive on ws 4. Focus/workspace stayed unchanged; cursor deltas each overlapped physical Razer event9 X/Y records. ~/.local/state/smartdock/dev-sessions/task8c/evidence/pointer-focus-gate-task8c-final.json |
