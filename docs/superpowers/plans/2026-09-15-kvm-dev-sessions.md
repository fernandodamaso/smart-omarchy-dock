...
| 2. Lifecycle/placement | INCONCLUSIVE (pointer-unchanged; not PASS) | Prior task2d UNQUALIFIED. Follow-up task8a: QEMU 1784255 / 0x5559cc4025b0 silent on ws 4; host focus stayed 0x5559cc3b7eb0/ws2 across guest input + 3 distinct grim digests; host cursor moved (user activity) so pointer clause cannot PASS. Guest stopped. ~/.local/state/smartdock/dev-sessions/task8a/evidence/pointer-focus-gate-task8a-final.json |
...
| 4. Guest compositor/input/capture | INCONCLUSIVE (pointer-unchanged; not PASS) | Prior task4a had changing frames with cursor drift. Follow-up task8a: per-color qs QML + grim digests 677c48ba / 10294fd9 / 1810e0ee while QEMU inactive on ws 4; focus unchanged; pointer INCONCLUSIVE (user cursor motion). Capture did not focus host QEMU. ~/.local/state/smartdock/dev-sessions/task8a/evidence/pointer-focus-gate-task8a-final.json |
